...
function generate_plankton!(plank, N::Int64, g::AbstractGrid, arch::Architecture,sp, maxN)
    ...
    for i in 1:N
        ...
        plank.data.z[i] = samples[index]
        plank.data.pool_z[i] = Int(ceil(plank.data.z[i],digits=0))/100 

        plank.data.length[i] = rand(plank.p.Min_Size[2][sp]:plank.p.Max_Size[2][sp])
        plank.data.length[i] = 40 #For experimental purposes.
    ...
    plank.data.x[1:N] .= [rand() * 1 for i in 1:N]
    plank.data.y[1:N] .= [rand() * 1 for i in 1:N]
        ...
        for i in 1:g.Nx
            for j in 1:g.Ny
                ...
                for k in 1:g.Nz
                    ...
                    density = sum(pdf_values[min_z:max_z]) .* groups.characters.Total_density[2][sp] / (max_z - min_z)

                    groups.density.num[k,] = density
                    groups.density.capacity[k,] = density
                end
            end
        end
    end
...
function reset(model::MarineModel)
    ...
    for (species_index,animal_index) in enumerate(keys(model.individuals.animals))
        ...
        if 6*60 <= model.t < 18*60
            samples = sample_normal(species.p.Day_depth_min[2][species_index],species.p.Day_depth_max[2][species_index],std = 20)
        else
            samples = sample_normal(species.p.Night_depth_min[2][species_index],species.p.Night_depth_max[2][species_index],std = 20)
        end
        ...
        for j in 1:n_ind
            if species.data.x[j] == -1 #Need to replace individual
                ...
                species.data.pool_z[j] = Int(ceil(species.data.z[j] / 100))
                species.data.x[j] = rand() * 1
                species.data.y[j] = rand() * 1
