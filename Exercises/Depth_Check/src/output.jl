function generate_outputs(model,spec,iterations,output_dt)
    ...
    consumption = zeros(spec, spec, model.grid.Nx,model.grid.Ny,model.grid.Nz,(output_dt+1)) #Subconsumption timestep
    consumption_aggregate = zeros(spec, spec, model.grid.Nx,model.grid.Ny,model.grid.Nz,Int(iterations/output_dt))
