...
function timestep_results(model,outputs,respire,ind_temp,species_index,ind)
    if species_index == 1
        outputs.timestep_array[ind,1,model.iteration] = model.individuals.animals[species_index].data.energy[ind]

        outputs.timestep_array[ind,2,model.iteration] = model.individuals.animals[species_index].data.weight[ind]

        outputs.timestep_array[ind,3,model.iteration] = model.individuals.animals[species_index].data.ration[ind]

        outputs.timestep_array[ind,4,model.iteration] = respire

        outputs.timestep_array[ind,5,model.iteration] = ind_temp

        
    else
        outputs.timestep_array[sum(model.ninds[1:(species_index-1)])+ind,1,model.iteration] = model.individuals.animals[species_index].data.energy[ind]
        
        outputs.timestep_array[sum(model.ninds[1:(species_index-1)])+ind,2,model.iteration] = model.individuals.animals[species_index].data.weight[ind]

        outputs.timestep_array[sum(model.ninds[1:(species_index-1)])+ind,3,model.iteration] = model.individuals.animals[species_index].data.ration[ind]

        outputs.timestep_array[sum(model.ninds[1:(species_index-1)])+ind,4,model.iteration] = respire

        outputs.timestep_array[sum(model.ninds[1:(species_index-1)])+ind,5,model.iteration] = ind_temp
    end
end
