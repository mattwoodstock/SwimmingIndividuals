function generate_outputs(model,spec,iterations,output_dt)
    ...
    behavior = zeros(sum(model.ninds),4,1) #Subconsumption timestep
    behavior_aggregate = zeros(sum(model.ninds),4,1)
    ...
    timestep_array = Array{Float64,3}(undef,sum(model.ninds),3,1)
    #daily_array = Array{Float64,4}(undef,sum(model.ninds),4,1). Fix later

    return MarineOutputs(mortalities,biomass,behavior,behavior_aggregate,consumption,consumption_aggregate,production,pool_density,timestep_array)
end
