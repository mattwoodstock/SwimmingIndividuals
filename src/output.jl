function generate_outputs(model,spec,iterations,output_dt)
    ...
    consumption = zeros(spec, spec, model.grid.Nx,model.grid.Ny,model.grid.Nz) #Subconsumption timestep
    population_results = Array{Float64,3}(undef,model.n_species,5,1)
    return MarineOutputs(mortalities,lengths,weights,consumption,population_results)
end